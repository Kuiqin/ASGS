#' BayesC
#' @description Calculates the Genomic Estimated Breeding Value based on BayesC method.
...
#' @return $fit BayesC model fitting
...
#' X=trout[1:20,2:11];Y=as.data.frame(trout[1:20,1]);r=0.1;ASGS::BC(X,Y,r)
BC<-function(X, Y, r){
  n<-nrow(X)
  p<-ncol(X)
  m<-round(n*r)
  ...
  for (i in 1:50){
    ...
    ETA<-list(list(X=as.matrix(X),model='BayesC',saveEffects=T))

    fm<-BGLR(y=yNA,ETA=ETA,nIter=12000,burnIn=2000,df0=5,S0=NULL,saveAt='BayesC_',verbose=F)
