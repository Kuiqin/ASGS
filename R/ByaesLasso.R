#' BayesLasso
#' @description Calculates the Genomic Estimated Breeding Value based on BayesLasso method.
...
#' @return $fit BayesL model fitting
...
#' X=trout[1:30,2:11];Y=as.data.frame(trout[1:30,1]);r=0.1;ASGS::BL(X,Y,r)
BL<-function(X, Y, r){
  n<-nrow(X)
  p<-ncol(X)
  m<-round(n*r)
  ...
  for (i in 1:50){
    ...
    ETA<-list(list(X=as.matrix(X),model='BL',saveEffects=T))

    fm<-BGLR(y=yNA,ETA=ETA,nIter=12000,burnIn=2000,df0=5,S0=NULL,saveAt='BL_',verbose=F)
