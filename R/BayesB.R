#' BayesB
#' @description Calculates the Genomic Estimated Breeding Value based on BayesB method.
...
#' @return $fit BayesB model fitting
...
#' X=trout[1:20,2:11];Y=as.data.frame(trout[1:20,1]);r=0.1;ASGS::BB(X,Y,r)
BB<-function(X, Y, r){
  n<-nrow(X)
  p<-ncol(X)
  m<-round(n*r)
  ...
  for (i in 1:50){
    ...
    ETA<-list(list(X=as.matrix(X),model='BayesB',saveEffects=T))

    fm<-BGLR(y=yNA,ETA=ETA,nIter=12000,burnIn=2000,df0=5,S0=NULL,saveAt='BayesB_',verbose=F)
